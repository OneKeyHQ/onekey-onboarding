...
const EXTENSION_DOWNLOAD_URL = {
  CHROME:
    'https://chrome.google.com/webstore/detail/onekey/infeboajgfhgbjpjbeppbkgnabfdkdaf',
  FIREFOX: 'https://addons.mozilla.org/zh-CN/firefox/addon/onekey-wallet/',
...
export default class Onboarding {
  ...
  _onMessage(event: MessageEvent) {
    ...
    if (event.data.type === 'onekey:reload') {
  ...
  /**
   * Starts onboarding by opening the OneKey download page and the Onboarding forwarder
   */
  startOnboarding() {
  ...
  /**
   * Checks whether the OneKey extension is installed
   */
  static isOneKeyInstalled() {
    return Boolean(
      (window as any).ethereum && (window as any).ethereum.isOneKey,
    );
  }
